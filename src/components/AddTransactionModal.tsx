...
export const AddTransactionModal: React.FC<AddTransactionModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (debtorMode === 'CREATE_NEW') {
      if (!newName.trim()) {
        setError('Vui lòng nhập tên người mới');
        return;
      }
      ...
      if (!targetDebtorId) {
        setError('Vui lòng chọn người mới');
        return;
      }
    }
      ...
      if (debtorMode === 'CREATE_NEW') {
        ...
        if (onSaveDebtor) {
          ...
          if (created && created.id) {
            ...
            setError('Không thể tạo người mới. Vui lòng thử lại.');
            return;
          }
        } else {
          setError('Hệ thống chưa hỗ trợ tạo người mới tại đây');
          return;
        }
      }
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0 overflow-hidden">
          ...
          <div className="space-y-2">
            <div className="flex flex-wrap items-center justify-between gap-1.5">
              <label className="font-semibold text-slate-700 text-xs sm:text-sm">
                Người giao dịch <span className="text-red-500">(*)</span>:
              </label>
            ...
            {debtorMode === 'SELECT' ? (
              debtors.length > 0 ? (
                <select
                  ...
                  className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-300 rounded-xl focus:bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500 font-medium text-slate-900 transition-all"
                >
                  {debtors.map((d) => (
              ...
              <div className="p-3.5 bg-emerald-50/80 border border-emerald-200 rounded-2xl space-y-3 animate-in fade-in">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                  <div>
                    ...
                    <div className="relative">
                      <input
                        ...
                        placeholder="VD: 7k9a..."
                        className="w-full pl-3 pr-8 py-2 bg-white border border-slate-300 rounded-xl text-xs font-mono font-bold text-slate-900 focus:outline-none focus:ring-2 focus:ring-emerald-500 tracking-wider"
                      />
                      <button
                        type="button"
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 bg-white border border-slate-300 rounded-xl text-xs text-slate-900 focus:outline-none focus:ring-2 focus:ring-emerald-500"
                    />
                  </div>
                </div>
                <p className="text-[11px] text-slate-500 leading-tight">
                  Bạn cấp mật khẩu này cho người này để họ tự vào tra cứu sao kê riêng (hỗ trợ cả chữ và số).
                </p>
              </div>
            )}
          </div>
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-300 focus:border-emerald-600 focus:ring-2 focus:ring-emerald-500/20 focus:bg-white rounded-xl text-xs sm:text-sm text-slate-900 outline-none transition-colors"
            />
          </div>
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                className="w-full pl-9 pr-3.5 py-2.5 bg-slate-50 border border-slate-300 focus:border-emerald-600 focus:ring-2 focus:ring-emerald-500/20 focus:bg-white rounded-xl text-xs sm:text-sm text-slate-900 outline-none transition-colors"
              />
          ...
          <div>
            ...
            {billImage ? (
              ...
              <label className="flex items-center justify-center gap-2 p-3.5 border border-dashed border-slate-300 hover:border-emerald-500 hover:bg-emerald-50/40 rounded-xl text-slate-600 hover:text-emerald-700 transition-colors cursor-pointer text-xs font-medium">
            ...
            {/* Sticky Action Footer - Cố định nút ở chân modal */}
            <div className="px-4 sm:px-6 py-3 bg-slate-50/95 backdrop-blur-xs border-t border-slate-200 flex items-center justify-end gap-2.5 shrink-0">
              ...
              <button
                ...
                disabled={isSubmitting}
                className="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 text-white font-bold rounded-xl text-xs sm:text-sm shadow-xs transition-colors cursor-pointer flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed disabled:pointer-events-none"
              >
